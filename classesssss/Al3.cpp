...
bool Al3::init()
{
    ...
    auto calcularQuimica = [](const auto& jugador1, const auto& jugador2) {
        int quimica = 60;
        if (get<7>(jugador1) == get<7>(jugador2)) quimica += 33; // país
        if (get<8>(jugador1) == get<8>(jugador2)) quimica += 33; //equipo
        if (get<9>(jugador1) == get<9>(jugador2)) quimica += 34; // liga
        return quimica;
        };

    float startX[] = { 700, 550, 550, 850 };
    float startY[] = { 710, 510, 310, 107 }; 
    float offsetX[] = { 300,200,200,200 };
    string posiciones[] = { "Delantero", "Medio", "Defensa", "Portero" };
    int cantidades[] = { 2, 4, 4, 1 };

    vector<decltype(JugadoresGlobales::jugadores)::mapped_type> jugadoresSeleccionados;

    for (int p = 0; p < 4; ++p) {
        string buscarPosicion = posiciones[p];
        auto rango = JugadoresGlobales::jugadores.equal_range(buscarPosicion);

        vector<decltype(JugadoresGlobales::jugadores)::mapped_type> candidatos;
        for (auto it = rango.first; it != rango.second; ++it) {
            candidatos.push_back(it->second);
        }

        for (int i = 0; i < min(cantidades[p], static_cast<int>(candidatos.size())); ++i) {
            int indiceAleatorio = rand() % candidatos.size();
            auto jugador = candidatos[indiceAleatorio];

            jugadoresSeleccionados.push_back(jugador);

            ratingTotal += get<10>(jugador);
            totalJugadores++;

            auto png = get<15>(jugador);
            auto nombre = get<0>(jugador);
    ...
    ratingTotal /= totalJugadores;
        ...
        ratingLabel = Label::createWithSystemFont("Rating: " + to_string((int)ratingTotal), "fonts/Novlang.ttf", 40);
        ratingLabel->setPosition(Vec2(origin.x + 200, 450));
        this->addChild(ratingLabel);
  
        quimicaLabel = Label::createWithSystemFont("Quimica: " + to_string((int)quimicaTotal) + "%", "fonts/Novlang.ttf", 40);
...
void Al3::menuCloseCallback(Ref* pSender)
{
